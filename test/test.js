...
describe('module loading with emptied require cache', () => {
  before(function () {
    //
    // A helper method to ensure that a module file
    // will be parsed/loaded again, thus code on the top
    // level of that file will be evaluated again.
    //
    this.enforceRequire = function enforceRequire(path) {
      var
        cachePath = require.resolve(path);
  ...
  describe('with amd/define available as global', function () {
    before(function () {
      define = function () {
        console.log('Hey, I am using fake define!!!');
      }
    ...
    it('should cover amd module loading', function () {
      const ToastMaker = this.enforceRequire('../lib/toastmaker');
    });
  });

  it('should cover common js module loading', function () {
    const ToastMaker = this.enforceRequire('../lib/toastmaker');
  });
});
